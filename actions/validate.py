...
expose_abnormal_pat1 = re.compile(r"[网站]点[^没不哪什么,，个]{,5}异常(?![件么嘛吗])|[网站]点[^没不哪什么，,个]{,5}有[^什么]{,2}问题(?![件么嘛吗是])|(?:圆通|快递站?|[网站]点|驿站)[^没不哪]{,7}(?:瘫痪|爆仓|整顿|调整)|已经[^没不哪]{,3}(?:瘫痪|爆仓)|[快包][件递裹][^没不]{,3}扣押|(?:业务员|快递|[网站]点|驿站)[^没不]{,5}罢工|老板[^没不怎]{,5}跑路")
expose_abnormal_pat1_skip = re.compile(r"没|不|签收|派送|帮|反馈|还|其他|上报|可以了|异常.{,3}问题|专门|正在|处理|专员|下工单|如果|假如|考核|商家|的问题")
