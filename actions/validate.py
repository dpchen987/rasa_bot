...
expose_abnormal_pat0 = re.compile(r"[网站]点.{,5}有.{,2}问题|(?:圆通|快递站?|[网站]点).{,5}(?:瘫痪|爆仓|整顿)|扣押.{,2}[快包][件递裹]|[快包][件递裹].{,3}扣押|(?:业务员|快递员?|[网站]点).{,5}罢工|老板.{,5}跑路|(?:圆通|你们公司).{,5}(?:破产|倒闭)")
...
expose_abnormal_pat0_skip = re.compile(r"没|不|哪|签收|派送|帮|反馈|还|其他|上报|可以了|异常.{,3}问题|专门|正在|处理|专员|下工单|如果|假如|考核|商家|的问题|之前|报备|备案|搬迁|恢复|正常了|单号|结束|之前|时[间候]|解释|对接|核实|情况|后续|跟进|安排|大概|菜鸟|驿站|操作|提示")
expose_abnormal_pat1_skip = re.compile(r"没|不|哪|签收|派送|帮|反馈|还|其他|上报|可以了|异常.{,3}问题|专门|正在|处理|专员|下工单|如果|假如|考核|商家|的问题|之前|报备|备案|搬迁|恢复|正常了|单号|结束|之前|时[间候]|解释|对接|核实|情况|后续|跟进|安排|大概|菜鸟|驿站|操作|提示|[是对][吧么吗嘛]|？")
